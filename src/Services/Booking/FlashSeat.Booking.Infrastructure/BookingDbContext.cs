...
public sealed class BookingDbContext(DbContextOptions<BookingDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<global::FlashSeat.Booking.Domain.Booking>(e => { e.ToTable("bookings"); e.HasKey(x => x.Id); e.HasIndex(x => x.BookingNumber).IsUnique(); e.HasIndex(x => x.HoldId).IsUnique(); e.HasIndex(x => x.UserId); e.Property(x => x.Status).HasConversion<string>(); e.Property(x => x.TotalAmount).HasPrecision(18, 2); e.Property(x => x.EventName).HasMaxLength(150); e.Property(x => x.EventSlug).HasMaxLength(160); e.Property(x => x.EventDescription).HasMaxLength(5000); e.Property(x => x.EventImageUrl).HasMaxLength(2048); e.Property(x => x.EventVenueName).HasMaxLength(200); e.Property(x => x.EventAddress).HasMaxLength(500); e.Property(x => x.EventStatus).HasMaxLength(40); e.Property(x => x.EventSnapshotAvailable); e.HasMany(x => x.Items).WithOne(x => x.Booking).HasForeignKey(x => x.BookingId); });
